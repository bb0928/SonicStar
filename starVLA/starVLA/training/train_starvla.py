...
from transformers import Adafactor, AutoProcessor, get_scheduler
...
if os.getenv("STARVLA_DISABLE_DEEPSPEED", "0") == "1":
...
def setup_optimizer_and_scheduler(model, cfg) -> Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler._LRScheduler]:
    ...
    optimizer_name = getattr(cfg.trainer.optimizer, "name", "AdamW")
    if optimizer_name == "AdamW":
        ...
        optimizer = Adafactor(
            ...
            lr=cfg.trainer.learning_rate.base,
            eps=(
                cfg.trainer.optimizer.eps,
                getattr(cfg.trainer.optimizer, "clip_threshold_eps", 1.0e-03),
            ),
            clip_threshold=getattr(cfg.trainer.optimizer, "clip_threshold", 1.0),
            decay_rate=getattr(cfg.trainer.optimizer, "decay_rate", -0.8),
            beta1=getattr(cfg.trainer.optimizer, "beta1", None),
            weight_decay=cfg.trainer.optimizer.weight_decay,
            scale_parameter=getattr(cfg.trainer.optimizer, "scale_parameter", False),
            relative_step=getattr(cfg.trainer.optimizer, "relative_step", False),
            warmup_init=getattr(cfg.trainer.optimizer, "warmup_init", False),
        )
    else:
        raise ValueError(f"Unsupported optimizer `{optimizer_name}`. Expected `AdamW` or `Adafactor`.")
...
class VLATrainer(TrainerUtils):
    ...
    def prepare_training(self, freeze_backbones=True):
...
def main(cfg) -> None:
    ...
    freeze_modules = cfg.trainer.freeze_modules if hasattr(cfg.trainer, "freeze_modules") else None
    ...
    trainer.prepare_training(freeze_backbones=False)
    trainer.train()
