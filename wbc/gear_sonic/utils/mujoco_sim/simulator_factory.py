...
def init_channel(config: Dict[str, Any]) -> None:
    ...
    """
    # Avoid duplicate initialization in the same process.
    # BaseSimulator may also attempt to initialize the channel.
    if config.get("__channel_factory_initialized", False):
        return
    ...
    config["__channel_factory_initialized"] = True
